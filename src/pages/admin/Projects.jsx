...
const Projects = () => {
  ...
  const handleAddProjectMembers = async () => {
    ...
    try {
      ...
      await addMemberToProject(
        ...
        userIds.map((user_id) => ({
          user_id,
          role_id: selectedUsers[user_id] || null,
        })),
      );
      setAlert({
        type: "success",
        message: `${userIds.length} member(s) added to project`,
      });
  ...
  const handleRemoveProjectMember = async (memberId, memberName, projectId) => {
    try {
      ...
      setAlert({
        type: "success",
        message: `${memberName} removed from project`,
      });
    } catch (err) {
      setAlert({
        type: "danger",
  ...
  const removeDb = (techIndex, dbIndex) => {
    setForm((prev) => {
      ...
      tech_details[techIndex] = {
        ...tech_details[techIndex],
        databases: tech_details[techIndex].databases.filter(
          (_, i) => i !== dbIndex,
        ),
      };
      return { ...prev, tech_details };
  ...
  const handleProjectTypeChange = (type, subtype, checked) => {
    setForm((prev) => {
      ...
      if (checked) {
        ...
        updated = current.filter((item) => item !== subtype);
      }
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-[2.5rem] p-16 text-center border border-dashed border-slate-200 shadow-2xl shadow-slate-200/40">
          ...
          <p className="text-slate-400 font-bold mt-2">
            Establish your first project to begin logging data.
          </p>
        </div>
      ) : (
        <>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/50">
                    <th className="px-10 py-6 text-sm font-black text-slate-400 uppercase tracking-[0.2em]">
                      Project{" "}
                    </th>
                    <th className="px-6 py-6 text-sm font-black text-slate-400 uppercase tracking-[0.2em]">
                      Code{" "}
                    </th>
                    ...
                    <th className="px-10 py-6 text-sm font-black text-slate-400 uppercase tracking-[0.2em] text-right">
                      Actions
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {filtered.map((project) => (
                    <>
                      <tr
                        key={project.id}
                        className={`hover:bg-slate-50/80 transition-colors group ${!project.is_active ? "opacity-60" : ""}`}
                      >
                        <td
                        ...
                        >
                          <div className="flex items-center gap-4">
                            <div
                              className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg shadow-inner transition-all ${project.is_active ? "bg-slate-50 text-[#132ea7] group-hover:bg-[#132ea7] group-hover:text-white" : "bg-slate-100 text-slate-400"}`}
                            >
                              <MdFolder size={24} />
                            </div>
                            <div>
                              ...
                              {project.description && (
                                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1.5 line-clamp-1 italic max-w-[250px]">
                                  {project.description}
                                </div>
                              )}
                            </div>
                        ...
                        <td className="px-6 py-5">
                          <span
                            className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${STATUS_COLORS[project.development_status] || "bg-slate-100 text-slate-600"}`}
                          >
                              ...
                              {!project.members?.length ? (
                                <div className="text-center py-6 bg-white rounded-2xl border border-dashed border-slate-200">
                                  ...
                                  <p className="text-slate-300 text-xs mt-1">
                                    Click "Add Members" to get started
                                  </p>
                                </div>
                              ) : (
                                    ...
                                    <div className="bg-white rounded-2xl border border-slate-100 p-5 space-y-4 shadow-sm">
                                      ...
                                      {availableEmps.length === 0 ? (
                                        <p className="text-slate-400 font-medium text-sm text-center py-4">
                                          All employees are already in this
                                          project
                                        </p>
                                      ) : (
                                        <>
                                                ...
                                                >
                                                  ...
                                                  >
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                      <p
                                                        className={`text-xs font-black truncate ${isSelected ? "text-[#132ea7]" : "text-slate-700"}`}
                                                      >
                                                        {emp.name}
                                                      </p>
                                                    ...
                                                    {isSelected ? (
                                                      <MdCheckCircle
                                                        size={18}
                                                        className="text-[#132ea7] flex-shrink-0"
                                                      />
                                                    ) : (
          ...
          <div className="md:hidden space-y-4">
            {filtered.map((project) => (
              <div
                key={project.id}
                className={`bg-white rounded-2xl p-4 border border-slate-100 shadow-sm space-y-3 ${!project.is_active ? "opacity-60" : ""}`}
              >
                {/* Header */}
                <div className="flex items-center gap-3">
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center font-black shrink-0 shadow-inner ${project.is_active ? "bg-[#132ea7]/10 text-[#132ea7]" : "bg-slate-100 text-slate-400"}`}
                  >
                    <MdFolder size={20} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-black text-slate-800 leading-tight uppercase truncate">
                      {project.name}
                    </p>
                    {/* <span className="inline-block mt-0.5 px-2 py-0.5 bg-[#132ea7]/10 text-[#132ea7] rounded-md text-[10px] font-black uppercase tracking-widest">
                      {project.code || "—"}
                    </span> */}
                  </div>
                ...
                <div className="space-y-2 text-sm pt-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-widest ${STATUS_COLORS[project.development_status] || "bg-slate-100 text-slate-600"}`}
                    >
                      {project.development_status || "—"}
                    </span>
                  </div>
                  ...
                  <div className="mt-3 bg-slate-50/50 rounded-xl p-3 border border-slate-100 space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        onClick={() =>
                          setAddingToProject(
                            addingToProject?.id === project.id ? null : project,
                          )
                        }
                        className="text-[#132ea7] hover:text-[#132ea7]/70 text-[8px] font-black uppercase tracking-widest transition-colors flex items-center gap-1"
                      >
                        {addingToProject?.id === project.id ? (
                          "Cancel Add"
                        ) : (
                          <>
                            <MdPersonAdd size={12} /> Add
                          </>
                        )}
                      </button>
                    </div>
                      ...
                      <div className="bg-white p-3 rounded-lg shadow-sm border border-slate-100 space-y-3">
                        <div className="space-y-2 max-h-[150px] overflow-y-auto">
                          {getAvailableEmployees(project).length === 0 ? (
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center py-2">
                    ...
                    <div className="space-y-2">
                      {project.members?.map((m) => (
                        <div
                          key={m.id}
                          className="flex items-center justify-between bg-white p-2 rounded-lg border border-slate-100 shadow-sm"
                        >
                          <div className="flex items-center gap-2">
                ...
                <div className="flex gap-2 pt-3 border-t border-slate-100">
                  ...
                  <button
                    onClick={() =>
                      navigate(`/admin/projects/${project.id}/dashboard`)
                    }
                    className="flex-1 h-10 rounded-xl bg-slate-50 text-slate-500 font-bold flex items-center justify-center gap-1.5 text-xs hover:bg-[#132ea7]/10 hover:text-[#132ea7] transition-all"
                  >
                    <MdDashboard size={16} />
...
/> */}
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Input
                          ...
                          value={tech.name}
                          onChange={(e) =>
                            updateTech(techIndex, "name", e.target.value)
                          }
                          className="flex-[2]"
                        />
                        <Input
                          ...
                          value={tech.version || ""}
                          onChange={(e) =>
                            updateTech(techIndex, "version", e.target.value)
                          }
                          className="flex-1"
                        />
                        <button
            ...
            <div>
              ...
              <div className="space-y-6">
                {Object.entries(PROJECT_TYPES).map(([type, subtypes]) => (
                  <div key={type} className="border rounded-2xl p-4">
                    <h4 className="font-bold capitalize mb-3">{type}</h4>
                      ...
                      {subtypes.map((subtype) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={
                              form.project_types?.[type]?.includes(subtype) ||
                              false
                            }
                            onChange={(e) =>
      ...
      />
          ...
          <div className="flex gap-4 pt-8 border-t border-slate-50">
            ...
            <Button
              type="submit"
              variant="primary"
              className="flex-[2] h-16 shadow-xl shadow-[#132ea7]/20 font-black uppercase tracking-[0.2em] text-sm"
              loading={submitting}
            >
                ...
                {Array.isArray(viewTarget.tech_details) ? (
                  <div className="space-y-2">
                    {viewTarget.tech_details.map((tech, index) => (
                      <div
                        key={index}
                        className="bg-[#132ea7] rounded-2xl p-4 text-white"
                      >
                        {/* Tech name + version */}
                        <div className="flex items-center gap-2">
