            ...
            <div className="bg-white rounded-[2rem] border border-slate-100 shadow-2xl shadow-slate-200/40 p-6 sm:p-8">
              ...
              <div className="mb-6">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <p className="text-[11px] font-black text-slate-500 uppercase tracking-widest">
                    {parseFloat(balance.used_paid)} /{" "}
                    {parseFloat(balance.entitled_paid)} used
                  </p>
                </div>
                <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${
                      parseFloat(balance.used_paid) >=
                      parseFloat(balance.entitled_paid)
                        ? "bg-red-500"
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  {
                    label: "Entitled",
                    value: parseFloat(balance.entitled_paid),
                    color: "bg-slate-50 text-slate-600",
                  },
                  {
                    label: "Paid Used",
                    value: parseFloat(balance.used_paid),
                    color: "bg-[#132ea7]/5 text-[#132ea7]",
                  },
                  {
                    label: "Unpaid",
                    value: parseFloat(balance.used_unpaid),
                    color:
                  ...
                  {
                    label: "Exchange",
                    value: parseFloat(balance.used_exchange),
              ...
              {parseFloat(balance.used_unpaid) > 0 && (
                <div className="mt-4 bg-red-50 border border-red-100 rounded-2xl px-4 py-3">
                  <p className="text-xs font-black text-red-500 uppercase tracking-widest">
                    ⚠️ {parseFloat(balance.used_unpaid)} day
                    {parseFloat(balance.used_unpaid) !== 1 ? "s" : ""} unpaid —
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {balanceHistory.map((b) => {
                      const remaining =
                        parseFloat(b.entitled_paid) - parseFloat(b.used_paid);
                      return (
                        <tr
                          key={b.id}
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <span
                              className={`font-black text-sm ${parseFloat(b.used_unpaid) > 0 ? "text-red-500" : "text-slate-400"}`}
                            >
                          ...
                          <td className="px-6 py-4">
                            <span
                              className={`px-3 py-1 rounded-lg text-[11px] font-black uppercase tracking-widest ${
                                remaining <= 0
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-col gap-1">
                            <Badge
                              ...
                              overrideColor={
                                LEAVE_TYPE_BADGE_MAP[leave.leave_type]
                              }
                            />
...
const Leaves = () => {
  ...
  useEffect(() => {
    const fetchCalculation = async () => {
      try {
        ...
        console.log("Leave Calculation:", response);
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      setAlert({
        type: "success",
        message: `Leave ${confirmApprove.display_id} approved.`,
      });
      ...
      if (userFilter) {
        const m = monthFilter
          ...
          : new Date().getMonth() + 1;
        const y = yearFilter ? parseInt(yearFilter) : new Date().getFullYear();
        getEmployeeBalance(userFilter, m, y).then((r) =>
          setEmpBalance(r.balance || null),
        );
      }
    } catch (err) {
      setAlert({
  ...
  const handleReject = async () => {
    ...
    if (rejectionReason.trim().length < 5) {
      setRejectionError("Must be at least 5 characters.");
      return;
    }
    try {
      setRejecting(true);
      ...
      setAlert({
        type: "success",
        message: `Leave ${rejectTarget.display_id} rejected.`,
      });
                        ...
                        >
                          ...
                          <td className="px-6 py-5">
                            <div className="font-black text-slate-800 text-base">
                              {formatDate(leave.created_at || leave.createdAt)}
                            </div>
                            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                              {
                                formatDateTime(
                                  leave.created_at || leave.createdAt,
                                ).split(" ")[1]
                              }
                            </div>
                          </td>
                ...
                >
                  ...
                  <div className="space-y-2 text-sm">
                    {[
                      ...
                      {
                        label: "Duration",
                        value: DURATION_LABELS[leave.duration],
                      },
                      { label: "From", value: formatDate(leave.start_date) },
                      { label: "To", value: formatDate(leave.end_date) },
                      {
                        label: "Requested",
                        value: formatDate(leave.created_at || leave.createdAt),
                      },
                    ].map((item) => (
                      <div
                        key={item.label}
          ...
          <div className="space-y-6 py-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                {
                  label: "Leave Type",
                  value: LEAVE_TYPE_LABELS[viewTarget.leave_type],
                },
                {
                  label: "Duration",
                ...
                { label: "To", value: formatDate(viewTarget.end_date) },
                {
                  label: "Requested",
                  value: formatDate(
                    viewTarget.created_at || viewTarget.createdAt,
                  ),
                },
                {
                  label: "Approved By",
        ...
        {rejectTarget && (
          <div className="space-y-5">
            <div className="bg-slate-50 rounded-2xl p-4">
              ...
              <p className="font-black text-slate-800">
                {rejectTarget.display_id}
              </p>
              <p className="text-sm font-bold text-slate-500 mt-1">
                {rejectTarget.employee?.name} —{" "}
                {LEAVE_TYPE_LABELS[rejectTarget.leave_type]}
              </p>
            </div>
            <Textarea
