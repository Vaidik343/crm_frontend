...
useEffect(() => {
  ...
  const filtered =
    filter === "all" ? calls : calls.filter((c) => c.call_type === filter);
  ...
  if (checked) {
    ...
    setForm((prev) => ({ ...prev, is_worklog: false }));
      ...
      const stillMatches = knownNames.some(
        (n) => n.toLowerCase() === newValue.toLowerCase(),
      );
      setForm((prev) => ({
        ...prev,
  ...
  const validate = () => {
    ...
    if (!form.caller_name.trim())
      errors.caller_name = "Caller name is required";
            ...
            >
              ...
              <div className="space-y-2 text-sm pt-1">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex flex-col items-end">
                    <span className="font-black text-slate-700 text-xs">
                      {call.caller_name || <MdPhone size={12} />}
                    </span>
                    {call.caller_number && (
                ...
                <div className="flex justify-between items-start gap-2">
                  <span className="text-slate-400 font-bold uppercase text-[10px] mt-1">
                    Type
                  </span>
        ...
        {form.client_id && (
          ...
          <div className="space-y-1.5">
            ...
            <LocalSearchableSelect
              ...
              getSearchText={(c) =>
                `${c.phone} ${(c.names || []).join(" ")} ${c.company || ""}`
              }
            />
            ...
            {selectedClient?.names?.length > 1 && (
              <div className="mt-2">
                ...
                <LocalSearchableSelect
                  options={selectedClient.names
                    .slice(1)
                    .map((n) => ({ name: n }))}
                  value={
                    form.caller_name !== selectedClient.names[0]
                      ? form.caller_name
                      : ""
                  }
                  onChange={(name) => {
                    if (name)
                      setForm((prev) => ({ ...prev, caller_name: name }));
                  }}
                  // emptyOptionLabel={selectedClient.names[0]}

                  emptyOptionLabel={
                    selectedClient.names[0]
                      ? "No Client"
                      : selectedClient.names?.[0]
                  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            />
            {/* {form.client_id && selectedClient?.email && (
    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1 mt-1">
        Email locked — update from the Clients page if needed
    </span>
)} */}
            ...
            {form.receive_type === "meeting" && (
              <div className="md:col-span-2 space-y-1.5">
                ...
                <MultiSearchableSelect
                  ...
                  onChange={(ids) =>
                    setForm((prev) => ({ ...prev, attendees: ids }))
                  }
                  getId={(u) => u.id}
          ...
          <option key={u.id} value={u.id}>
            {u.name} ({u.employee_id}){roleLabel ? ` — ${roleLabel}` : ""}
                    ...
                    <LocalSearchableSelect
                      ...
                      value={form.transfer_to}
                      onChange={(id) =>
                        setForm((prev) => ({ ...prev, transfer_to: id }))
                      }
