...
const AdminInternDetail = () => {
  ...
  const handleUpdateMentor = async (e) => {
    ...
    try {
      ...
      await adminUpdateProject(id, { mentor_id: mentorId || null });
    ...
    <div className="flex flex-col gap-6">
              ...
              {projectLoading ? (
                ...
                <>
                  ...
                  <div className="flex items-center justify-between gap-4 pt-4 border-t border-slate-100">
                    <div>
                      <p className={fieldLbl}>Mentor</p>
                      <p className={fieldVal}>
                        {project.mentor?.name || <span className="italic text-slate-400">Not assigned</span>}
                      </p>
                    ...
                    <button
                      onClick={() => { setMentorId(project.mentor?.id || ""); setShowMentor(true); }}
      ...
      {showMentor && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <h2 className="text-sm font-black uppercase tracking-widest text-slate-700">Update Mentor</h2>
              <button onClick={() => setShowMentor(false)} className="text-slate-400 hover:text-slate-600 transition">
                <MdClose size={20} />
              </button>
            </div>
            <form onSubmit={handleUpdateMentor} className="px-6 py-6 flex flex-col gap-5">
              <div>
                <label className={labelCls}>Select Mentor</label>
                <LocalSearchableSelect
                  options={users}
                  value={mentorId}
                  onChange={(id) => setMentorId(id)}
                  getId={(u) => u.id}
                  getLabel={getMemberLabel}
                  getSearchText={(u) => `${u.name} ${u.employee_id}`}
                  placeholder="Search employee..."
