...
// ── Form Fields Component (Defined OUTSIDE to prevent losing input focus) ──────

const WorkLogFormFields = ({ values, onChange, errors, project, tasks = [] }) => {
  const inputCls = (err) =>
    ...
    <>
      ...
      <div>
        <label className={labelCls}>
          Project <span className="text-slate-400 font-medium normal-case tracking-normal">(optional)</span>
        </label>
      ...
      <div>
        <label className={labelCls}>
          Task <span className="text-slate-400 font-medium normal-case tracking-normal">(optional)</span>
        </label>
...
const InternWorkLogs = () => {
  ...
  // ── Total hours safe sum ───────────────────────────────────────────────────
  const totalHoursOnPage = (workLogs || []).reduce(
    (sum, w) => sum + parseFloat(w?.hours || 0), 
    0
  );
...
const validateCreate = () => {
  ...
  // ✅ Only validate numerical value IF hours has been provided
...
const validateEdit = () => {
  ...
  // ✅ Only validate numerical value IF hours has been provided
  if (editForm.hours && (isNaN(parseFloat(editForm.hours)) || parseFloat(editForm.hours) <= 0)) {
    e.errors = "Enter a valid positive number.";
  }
                ...
                <tbody>
                  {(workLogs || []).map((w) => (
                    <tr key={w?.id} className="border-b border-slate-50 hover:bg-slate-50 transition">
                      ...
                      <td className="px-4 py-3 text-xs font-semibold text-slate-500 whitespace-nowrap">
                        {w?.task?.display_id ? w.task.display_id : "—"}
                      </td>
                      <td className="px-4 py-3 text-xs font-semibold text-slate-500 whitespace-nowrap">
                        {w?.log_date ? formatDate(w.log_date) : "—"}
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-sm font-black text-[#132ea7]">
                          {parseFloat(w?.hours || 0).toFixed(1)} hrs
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <button
              ...
              {(workLogs || []).map((w) => (
                <div key={w?.id} className="p-4 flex flex-col gap-2">
                  ...
                  <div className="flex items-center gap-3 text-xs text-slate-400 font-semibold flex-wrap">
                    <span>📅 {w?.log_date ? formatDate(w.log_date) : "—"}</span>
                    {w?.project?.name && <span>📁 {w.project.name}</span>}
                    {w?.task?.display_id && <span>✅ {w.task.display_id}</span>}
                  </div>
                  <button
                    onClick={() => openEdit(w)}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreate} className="px-6 py-6 flex flex-col gap-5">
              <WorkLogFormFields values={form} onChange={handleFormChange} errors={formErrors} project={project} tasks={tasks} />
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleEdit} className="px-6 py-6 flex flex-col gap-5">
              <WorkLogFormFields values={editForm} onChange={handleEditChange} errors={editErrors} project={project} tasks={tasks} />
              <div className="flex gap-3 pt-2">
